...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      ...
      const user = await storage.createUser({
        username,
        password, // In a real app, we should hash the password
        tier: "free"
      });
  ...
  app.post("/api/subscribe", async (req, res) => {
    try {
      ...
      // In a real app, get user ID from session. For demo, use a default.
      const userId = 1;
        ...
        line_items: [
          {
            price_data: {
              currency: 'usd',
              product_data: {
                name: 'InvoiceLyticsBot Premium Subscription',
                description: 'Unlimited invoices, detailed analytics, and premium support'
              },
  ...
  app.get('/subscription-success', async (req, res) => {
    try {
      ...
      if (!session) {
        return res.redirect('/dashboard?error=session-not-found');
      }
      ...
      if (!subscriptionId) {
        return res.redirect('/dashboard?error=subscription-not-found');
      }
      ...
      await storage.updateUserTier(userId, 'premium');
      ...
      // Reset usage for the new premium user
      await storage.resetUserUsage(userId);
      
      return res.redirect('/dashboard?success=subscription-active');
    } catch (error) {
      console.error("Error processing subscription success:", error);
      return res.redirect('/dashboard?error=subscription-processing');
    }
  });
